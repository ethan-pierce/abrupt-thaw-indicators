...
print(f"  Abrupt thaw predictions: {(predictions == 1).sum():,} ({(predictions == 1).sum()/n_pixels*100:.1f}%)")
print(f"  Gradual thaw predictions: {(predictions == 0).sum():,} ({(predictions == 0).sum()/n_pixels*100:.1f}%)")
...
output_ds = xr.Dataset(
    ...
    attrs={
        ...
        'probability_description': 'Probability of abrupt thaw (class 1)',
        'prediction_description': 'Binary prediction: 0=Gradual Thaw, 1=Abrupt Thaw',
...
ax.set_title('Abrupt Thaw Probability Map', fontsize=14, fontweight='bold')
