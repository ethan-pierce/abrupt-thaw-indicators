...
DISPLAY_MIN = 0.01          # families below this in-AOA share fold into "Other"
...
def load_cache():
    ...
    return (d["dominant_family"], list(d["families"]), d["area_fraction"],
            str(d["gate_top_family"]), float(d["gate_top_fraction"]), bool(d["gate_ok"]))
...
def build_display_codes(fam_raster, families, fracs):
    ...
    disp = [families[i] for i in order if fracs[i] >= DISPLAY_MIN]
    disp_idx = {d: i for i, d in enumerate(disp)}
    K = len(disp)
    other_code, mask_code = K, K + 1

    fam_to_disp = np.array([disp_idx.get(d, other_code) for d in families], dtype=int)

    return disp, disp_idx, K, other_code, mask_code, fam_to_disp, order


def coded_raster(fam_raster, valid, fam_to_disp, mask_code):
    ny, nx = fam_raster.shape
    coded = np.full((ny, nx), np.nan, dtype="float64")
    in_aoa = fam_raster >= 0
    coded[valid & ~in_aoa] = mask_code                       # reliability backdrop
    ys, xs = np.nonzero(in_aoa)
    coded[ys, xs] = fam_to_disp[fam_raster[ys, xs]]          # attributed family / Other
    return coded
...
def area_fraction_panel(ax, disp, disp_idx, fracs, families, order):
    ...
    colors = [fd.COLORS[d] for d in disp]
    if other_share > 0:
...
def main():
    figstyle.use()
    fam_raster, families, fracs, gate_fam, gate_frac, gate_ok = load_cache()
    ...
    gs_bar = fig.add_gridspec(1, 1, left=0.815, right=0.975, top=0.86, bottom=0.155)
